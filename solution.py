...
def build_packet():
    ...
    chcksum = 0
    ID = os.getpid() & 0xFFFF
    header = struct.pack("bbHHh", ICMP_ECHO_REQUEST, 0, chcksum, ID, 1)
    data = struct.pack("d", time.time())
    chcksum = checksum(header + data)

    if sys.platform == 'darwin':
        chcksum = socket.htons(chcksum) & 0xffff
        ...
        for tries in range(TRIES):
            ...
            try:
                ...
                try: #try to fetch the hostname
                    #Fill in start
                    myHostname = gethostbyaddr(addr[0])[0]
                    #Fill in end
                except herror:   #if the host does not provide a hostname
                    #Fill in start
                    myHostname = "Hostname not unreturnable"
                    #Fill in end
